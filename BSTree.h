...
class BSTree                                     // KeyType : key field
{
  ...
  protected:
	...
	bool retrieveHelper(const KeyType& searchKey, DataType& searchDataItem, BSTreeNode *&p);
	void writeKeysHelper(BSTreeNode *p) const;
	void clearHelper(BSTreeNode *p);
	int getHeightHelper(BSTreeNode *p, int height) const;
	int getCountHelper(BSTreeNode *p, int &count) const;
	...
	void BSTreeCopyHelper(BSTreeNode *p1, BSTreeNode *&p2);
    // Data member
